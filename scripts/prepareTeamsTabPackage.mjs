...
const [domain, teamsAppId] = process.argv.slice(2);
if (!domain || !teamsAppId) {
  console.error("Usage: node scripts/prepareTeamsTabPackage.mjs <public-domain> <teams-app-id>");
  process.exit(1);
}
...
let manifest = fs.readFileSync(path.join(src, "manifest.tab-only.template.json"), "utf8");
manifest = manifest
...
for (const icon of ["color.png", "outline.png"]) {
  fs.copyFileSync(path.join(src, icon), path.join(out, icon));
}
